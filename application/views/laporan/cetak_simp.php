...
<body>
	...
	<table class="items" width="100%" style="font-size: 9pt; font-family: serif; border-collapse: collapse; " cellpadding="8">
		...
		<tbody>
			<!-- ITEMS HERE -->
			<?php $i = 1; ?>
			<?php foreach ($data['simpanan'] as $simp) : ?>
			<tr>
				<th scope="row"><?= $i; ?></th>
				<td><?= 'Rp.'.number_format($simp['deposit'],2,'.',','); ?></td>
				<td><?= date_format(date_create($simp['tanggal']), 'd F Y') ?></td>
				<td><img src="<?= base_url('assets/img/simpanan/').$simp['bukti'];?>" width="50" height="50" alt=""></td>
			</tr>
			<?php $i++; ?>
			<?php endforeach; ?>
		</tbody>
